      ...
      <ul className="list-none items-center hidden md:flex justify-end flex-1">
        {navLinks.map((nav, index) => (
          ...
          >
            <Link
              to={`${nav.path}`}
              className="text-slate-600 font-normal font-poppins tracking-wider px-3 py-2 rounded-md text-sm"
            >
              {nav.name}
            </Link>
          </li>
        ...
        >
          <ul className="list-none items-center flex flex-col justify-end flex-1">
            {navLinks.map((nav, index) => (
              <li
                key={nav.id}
                className={`font-poppins font-semibold tracking-wider cursor-pointer text-[16px] ${
                  index === navLinks.length - 1 ? "mr-0" : "mb-4"
                } text-slate-600 mr-10`}
              >
